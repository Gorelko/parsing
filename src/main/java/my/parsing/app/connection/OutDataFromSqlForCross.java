...
public class OutDataFromSqlForCross {

    public static List<PriceDto> outDataFromSql(HttpServletRequest request, List<String> listFilterData, String arrDel[]) {
            ...
            if (listForFilter.size() == 0) {
                ...
                if (arrDel != null) {
                    ...
                    for (int i = 0; i < arrDel.length; i++) {
                        statement.executeUpdate("DELETE FROM training_db.amtel_price WHERE id = " + arrDel[i]);
                    }
                }
